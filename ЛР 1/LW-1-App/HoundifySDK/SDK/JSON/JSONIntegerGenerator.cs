...
public abstract class JSONIntegerGenerator : JSONCheckingHandler
  {
    protected JSONIntegerGenerator()
      {
      }
    ...
    public override void boolean_value(bool to_write)
      {
        error("Expected an integer value for %what%, found {0}.",
              (to_write ? "true" : "false"));
      }
    ...
    public override void string_value(string to_write)
      {
        error("Expected an integer value for %what%, found the string \"{0}\".",
              to_write);
      }
