/* file "JSONObjectGenerator.cs" */

/* Copyright 2015, 2016 SoundHound, Incorporated.  All rights reserved. */

using System.Numerics;
using System.Diagnostics;


public abstract class JSONObjectGenerator : JSONCheckingHandler
  {
    private int depth;
    private string field_name;
    private JSONHandler field_handler;

    private void post_value()
      {
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        if (depth == 1)
          {
            finish_field(field_name, field_handler);
            field_name = null;
            field_handler = null;
          }
      }

    protected JSONObjectGenerator()
      {
        depth = 0;
        field_name = null;
        field_handler = null;
      }

    protected abstract void start();
    protected abstract JSONHandler start_field(string field_name);
    protected abstract void finish_field(string field_name,
                                         JSONHandler field_handler);
    protected abstract void finish();

    public override void true_value()
      {
        if (depth == 0)
            error("Expected an object value for %what%, found true.");
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.true_value();
        post_value();
      }

    public override void false_value()
      {
        if (depth == 0)
            error("Expected an object value for %what%, found false.");
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.false_value();
        post_value();
      }

    public override void boolean_value(bool to_write)
      {
        if (depth == 0)
          {
            error("Expected an object value for %what%, found {0}.",
                  (to_write ? "true" : "false"));
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.boolean_value(to_write);
        post_value();
      }

    public override void null_value()
      {
        if (depth == 0)
            error("Expected an object value for %what%, found null.");
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.null_value();
        post_value();
      }

    public override void string_value(string to_write)
      {
        if (depth == 0)
          {
            error("Expected an object value for %what%, found the string " +
                  "\"{0}\".", to_write);
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.string_value(to_write);
        post_value();
      }

    public override void number_value(int to_write)
      {
        if (depth == 0)
          {
            error("Expected an object value for %what%, found the integer {0}.",
                  to_write);
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.number_value(to_write);
        post_value();
      }

    public override void number_value(long to_write)
      {
        if (depth == 0)
          {
            error("Expected an object value for %what%, found the integer {0}.",
                  to_write);
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.number_value(to_write);
        post_value();
      }

    public override void number_value(BigInteger to_write)
      {
        if (depth == 0)
            bad_integer(to_write, "an object value");
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.number_value(to_write);
        post_value();
      }

    public override void number_value(double to_write, int precision)
      {
        if (depth == 0)
          {
            error("Expected an object value for %what%, found the rational " +
                  string.Format("{{0:f{0}}}.", precision), to_write);
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.number_value(to_write, precision);
        post_value();
      }

    public override void number_value(BigInteger mantissa_whole_part,
            BigInteger mantissa_fraction_numerator,
            int mantissa_fraction_digit_count, BigInteger exponent)
      {
        if (depth == 0)
          {
            bad_number(mantissa_whole_part, mantissa_fraction_numerator,
                    mantissa_fraction_digit_count, exponent, "an object value");
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.number_value(mantissa_whole_part,
                mantissa_fraction_numerator, mantissa_fraction_digit_count,
                exponent);
        post_value();
      }

    public override void number_value(string number_text)
      {
        if (depth == 0)
          {
            error("Expected an object value for %what%, found the number {0}.",
                  number_text);
          }
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.number_value(number_text);
        post_value();
      }

    public override void start_object()
      {
        if (depth == 0)
          {
            Debug.Assert(field_name == null);
            Debug.Assert(field_handler == null);
            start();
          }
        else
          {
            Debug.Assert(field_name != null);
            Debug.Assert(field_handler != null);
            field_handler.start_object();
          }
        ++depth;
      }

    public override void start_pair(string name)
      {
        Debug.Assert(name != null);

        Debug.Assert(depth > 0);
        if (depth == 1)
          {
            Debug.Assert(field_name == null);
            Debug.Assert(field_handler == null);
            field_name = name;
            field_handler = start_field(field_name);
            Debug.Assert(field_handler != null);
          }
        else
          {
            Debug.Assert(field_name != null);
            Debug.Assert(field_handler != null);
            field_handler.start_pair(name);
          }
      }

    public override void pair(string name, string new_string_value)
      {
        start_pair(name);
        string_value(new_string_value);
      }

    public override void pair(string name, bool value)
      {
        start_pair(name);
        boolean_value(value);
      }

    public override void pair(string name, int value)
      {
        start_pair(name);
        number_value(value);
      }

    public override void pair(string name, long value)
      {
        start_pair(name);
        number_value(value);
      }

    public override void pair(string name, double value, int precision)
      {
        start_pair(name);
        number_value(value, precision);
      }

    public override void finish_object()
      {
        Debug.Assert(depth > 0);
        --depth;
        if (depth == 0)
          {
            Debug.Assert(field_name == null);
            Debug.Assert(field_handler == null);
            finish();
          }
        else
          {
            Debug.Assert(field_name != null);
            Debug.Assert(field_handler != null);
            field_handler.finish_object();
            post_value();
          }
      }

    public override void start_array()
      {
        if (depth == 0)
            error("Expected an object value for %what%, found an array.");
        ++depth;
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.start_array();
      }

    public override void finish_array()
      {
        Debug.Assert(depth > 1);
        --depth;
        Debug.Assert(field_name != null);
        Debug.Assert(field_handler != null);
        field_handler.finish_array();
        post_value();
      }

    public override void reset()
      {
        depth = 0;
        field_name = null;
        field_handler = null;
      }
  };
