...
public class NavigationCancelCommandJSON : NavigationCommandJSON
  {
    ...
    public virtual int extraNavigationCancelCommandComponentCount()
      {
        Debug.Assert(extraKeys.Count == extraValues.Count);
        return extraKeys.Count;
      }
    public virtual string extraNavigationCancelCommandComponentKey(int component_num)
      {
        Debug.Assert(extraKeys.Count == extraValues.Count);
        Debug.Assert(component_num < extraValues.Count);
        return extraKeys[component_num];
      }
    public virtual JSONValue extraNavigationCancelCommandComponentValue(int component_num)
      {
        Debug.Assert(extraKeys.Count == extraValues.Count);
        Debug.Assert(component_num < extraValues.Count);
        return extraValues[component_num];
      }
    public virtual JSONValue extraNavigationCancelCommandLookup(string field_name)
      {
    ...
    public override int extraNavigationCommandComponentCount()
      {
        int result = 0;
        result += extraNavigationCancelCommandComponentCount();
        return result;
      }
    public override string extraNavigationCommandComponentKey(int component_num)
      {
        return extraNavigationCancelCommandComponentKey(component_num);
      }
    public override JSONValue extraNavigationCommandComponentValue(int component_num)
      {
        return extraNavigationCancelCommandComponentValue(component_num);
      }
    public override JSONValue extraNavigationCommandLookup(string field_name)
      {
        return extraNavigationCancelCommandLookup(field_name);
      }


    public virtual void extraNavigationCancelCommandAppendPair(string key, JSONValue new_component)
      {
    ...
    public virtual void extraNavigationCancelCommandSetField(string key, JSONValue new_component)
      {
        Debug.Assert(key != null);
        Debug.Assert(new_component != null);

        JSONValue old_field = extraNavigationCancelCommandLookup(key);
        if (old_field == null)
          {
            extraNavigationCancelCommandAppendPair(key, new_component);
          }
        else
          {
    ...
    public override void extraNavigationCommandSetField(string key, JSONValue new_component)
      {
        extraNavigationCancelCommandSetField(key, new_component);
      }
        ...
        protected override void finish()
          {
            ...
            for (int extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result.extraNavigationCancelCommandAppendPair(unknownFieldGenerator.field_names[extra_num], unknownFieldGenerator.field_values[extra_num]);
              }
