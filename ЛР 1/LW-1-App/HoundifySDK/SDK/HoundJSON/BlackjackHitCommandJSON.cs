/* file "BlackjackHitCommandJSON.cs" */
...
public class BlackjackHitCommandJSON : BlackjackCommandJSON
  {
    public class TypeNativeDataJSON : JSONBase
        ...
        public static TypeNativeDataJSON from_json(JSONBase json_value, bool ignore_extras, bool allow_incomplete)
      {
        ...
        private class FieldHoldingGeneratorDealerHandTotal : JSONHoldingIntegerRangeGenerator
              {
                public FieldHoldingGeneratorDealerHandTotal(String what) : base(what, 2, 26)
                  {
                  }
              };
        private class FieldHoldingArrayGeneratorDealerHandTotal : JSONHoldingIntegerRangeArrayGenerator
              {
                public FieldHoldingArrayGeneratorDealerHandTotal(String what) : base(what, 2, 26)
                  {
                  }
              };
    ...
    public BlackjackHitCommandJSON()
      {
    ...
    public virtual int extraBlackjackHitCommandComponentCount()
      {
        Debug.Assert(extraKeys.Count == extraValues.Count);
        return extraKeys.Count;
      }
    public virtual string extraBlackjackHitCommandComponentKey(int component_num)
      {
        Debug.Assert(extraKeys.Count == extraValues.Count);
        Debug.Assert(component_num < extraValues.Count);
        return extraKeys[component_num];
      }
    public virtual JSONValue extraBlackjackHitCommandComponentValue(int component_num)
      {
        Debug.Assert(extraKeys.Count == extraValues.Count);
        Debug.Assert(component_num < extraValues.Count);
        return extraValues[component_num];
      }
    public virtual JSONValue extraBlackjackHitCommandLookup(string field_name)
      {
        JSONValue result = (extraIndex.ContainsKey(field_name) ? extraIndex[field_name] : null);
        return result;
      }
    public override int extraBlackjackCommandComponentCount()
      {
        ...
        if (flagHasNativeData)
            ++result;
        result += extraBlackjackHitCommandComponentCount();
        return result;
      }
    public override string extraBlackjackCommandComponentKey(int component_num)
      {
        ...
        return extraBlackjackHitCommandComponentKey(remainder);
      }
    public override JSONValue extraBlackjackCommandComponentValue(int component_num)
      {
        ...
        return extraBlackjackHitCommandComponentValue(remainder);
      }
    public override JSONValue extraBlackjackCommandLookup(string field_name)
      {
        ...
        return extraBlackjackHitCommandLookup(field_name);
      }
    ...
    public virtual void extraBlackjackHitCommandAppendPair(string key, JSONValue new_component)
      {
        Debug.Assert(key != null);
        Debug.Assert(new_component != null);
    ...
    public virtual void extraBlackjackHitCommandSetField(string key, JSONValue new_component)
      {
        Debug.Assert(key != null);
        Debug.Assert(new_component != null);

        JSONValue old_field = extraBlackjackHitCommandLookup(key);
        if (old_field == null)
          {
            extraBlackjackHitCommandAppendPair(key, new_component);
          }
        else
          {
            int count = extraKeys.Count;
    ...
    public override void extraBlackjackCommandSetField(string key, JSONValue new_component)
      {
        ...
        extraBlackjackHitCommandSetField(key, new_component);
      }
    ...
    public static new BlackjackHitCommandJSON from_json(JSONValue json_value, bool ignore_extras, bool allow_incomplete, bool allow_unpolished)
      {
        BlackjackHitCommandJSON result;
          {
            HoldingGenerator generator = new HoldingGenerator("Type BlackjackHitCommand", ignore_extras);
            generator.set_allow_incomplete(allow_incomplete);
            generator.set_allow_unpolished(allow_unpolished);
            if (allow_incomplete || allow_unpolished)
                json_value.write(generator);
    ...
    public static new BlackjackHitCommandJSON from_json(JSONValue json_value, bool ignore_extras, bool allow_incomplete)
  {
    return from_json(json_value, ignore_extras, allow_incomplete, false);
  }
    public static new BlackjackHitCommandJSON from_json(JSONBase json_value, bool ignore_extras, bool allow_incomplete, bool allow_unpolished)
      {
        BlackjackHitCommandJSON result;
          {
            HoldingGenerator generator = new HoldingGenerator("Type BlackjackHitCommand", ignore_extras);
    ...
    public static new BlackjackHitCommandJSON from_json(JSONBase json_value, bool ignore_extras, bool allow_incomplete)
  {
    return from_json(json_value, ignore_extras, allow_incomplete, false);
  }
    public static new BlackjackHitCommandJSON from_text(string text, bool ignore_extras)
      {
        BlackjackHitCommandJSON result;
          {
            HoldingGenerator generator = new HoldingGenerator("Type BlackjackHitCommand", ignore_extras);
            JSONParse.parse_json_value(text, "Text for BlackjackHitCommandJSON", generator);
            Debug.Assert(generator.have_value);
            result = generator.value;
          };
        return result;
      }
    public static new BlackjackHitCommandJSON from_file(TextReader fp, string file_name, bool ignore_extras)
      {
        BlackjackHitCommandJSON result;
          {
            HoldingGenerator generator = new HoldingGenerator("Type BlackjackHitCommand", ignore_extras);
    ...
    public new abstract class Generator : BlackjackCommandJSON.Generator
      {
        ...
        protected override void finish()
          {
            if (!(getBlackjackCommandJSONKey().Equals("BlackjackHitCommand")))
                throw new Exception("The key field has a value other than `BlackjackHitCommand'.");
            BlackjackHitCommandJSON result = new BlackjackHitCommandJSON();
            Debug.Assert(result != null);
            finish(result);
            int extra_count = unknownFieldGenerator.field_names.Count;
            Debug.Assert(extra_count == unknownFieldGenerator.field_values.Count);
            for (int extra_num = 0; extra_num < extra_count; ++extra_num)
              {
                result.extraBlackjackHitCommandAppendPair(unknownFieldGenerator.field_names[extra_num], unknownFieldGenerator.field_values[extra_num]);
              }
            unknownFieldGenerator.field_names.Clear();
            unknownFieldGenerator.field_values.Clear();
            unknownFieldGenerator.index = new Dictionary<string, JSONValue >();
        ...
        protected override void handle_result(BlackjackCommandJSON new_result)
          {
            handle_result((BlackjackHitCommandJSON )new_result);
          }
        protected void finish(BlackjackHitCommandJSON result)
          {
        ...
        protected abstract void handle_result(BlackjackHitCommandJSON new_result);
        ...
        public Generator(bool ignore_extras) : base(ignore_extras)
          {
            fieldGeneratorNativeData = new TypeNativeDataJSON.HoldingGenerator("field \"NativeData\" of the BlackjackHitCommand class", ignore_extras);
            unknownFieldGenerator = new UnknownFieldGenerator(ignore_extras);
            set_what("the BlackjackHitCommand class");
          }
        public Generator() : base(false)
          {
            fieldGeneratorNativeData = new TypeNativeDataJSON.HoldingGenerator("field \"NativeData\" of the BlackjackHitCommand class", false);
            unknownFieldGenerator = new UnknownFieldGenerator(false);
            set_what("the BlackjackHitCommand class");
          }
    ...
    public new class HoldingGenerator : Generator
      {
        protected override void handle_result(BlackjackHitCommandJSON  result)
          {
//@@@            Debug.Assert(!have_value);
            have_value = true;
            value = result;
        ...
        public bool have_value;
        public BlackjackHitCommandJSON value;
      };
    public new class HoldingArrayGenerator : JSONArrayGenerator
  {
    ...
    protected virtual void handle_result(List<BlackjackHitCommandJSON> result)
    ...
    public HoldingArrayGenerator(string what, bool ignore_extras)
      {
        element_handler = new ElementHandler(this, ignore_extras);
        have_value = false;
        value = new List<BlackjackHitCommandJSON>();
        base.set_what(what);
      }
    public HoldingArrayGenerator(bool ignore_extras)
      {
    ...
    public bool have_value;
    public List<BlackjackHitCommandJSON> value;
  };
  };
