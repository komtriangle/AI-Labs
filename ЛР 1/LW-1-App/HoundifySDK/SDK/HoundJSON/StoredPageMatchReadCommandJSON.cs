...
public class StoredPageMatchReadCommandJSON : StoredPageMatchCommandJSON
  {
    ...
    private JSONValue  extraPageIsOnByDefaultToJSON()
      {
        JSONValue generated_boolean_PageIsOnByDefault = (storePageIsOnByDefault ? (JSONValue )new JSONTrueValue() : (JSONValue )new JSONFalseValue());
        return generated_boolean_PageIsOnByDefault;
      }
        ...
        if (json_true != null)
        ...
        else
          {
            ...
            if (json_false != null)
              ...
              {
                throw new Exception("The value for field PageIsOnByDefault of StoredPageMatchReadCommandJSON is not true for false.");
              }
          }
    ...
    public override JSONValue extraStoredPageMatchCommandComponentValue(int component_num)
      {
        ...
        if (flagHasPageIsOnByDefault)
          {
            if (remainder == 0)
                return extraPageIsOnByDefaultToJSON();
            --remainder;
          }
        if (flagHasPageData)
          {
            ...
            switch (field_name[4])
              {
                case 'D':
                    ...
                    break;
                case 'I':
                    if (String.Compare(field_name, 5, "s", 0, 1, false) == 0)
                      {
                        switch (field_name[6])
                          {
                            case 'G':
                                if ((String.Compare(field_name, 7, "lobal", 0, 5, false) == 0) && (field_name.Length == 12))
                                    return (flagHasPageIsGlobal ? extraPageIsGlobalToJSON() : null);
                                break;
                            case 'O':
                                if ((String.Compare(field_name, 7, "nByDefault", 0, 10, false) == 0) && (field_name.Length == 17))
                                    return (flagHasPageIsOnByDefault ? extraPageIsOnByDefaultToJSON() : null);
                                break;
                            default:
                                break;
                          }
            ...
            switch (key[4])
              {
                case 'D':
                    ...
                    break;
                case 'I':
                    if (String.Compare(key, 5, "s", 0, 1, false) == 0)
                      {
                        switch (key[6])
                          {
                            case 'G':
                                if ((String.Compare(key, 7, "lobal", 0, 5, false) == 0) && (key.Length == 12))
                                    {
                                ...
                                break;
                            case 'O':
                                if ((String.Compare(key, 7, "nByDefault", 0, 10, false) == 0) && (key.Length == 17))
                                    {
                                    fromJSONPageIsOnByDefault(new_component, false);
                                    return;
                                    }
                                break;
                            default:
        ...
        public Generator(bool ignore_extras) : base(ignore_extras)
          {
            ...
            fieldGeneratorPageIsOnByDefault = new JSONHoldingBooleanGenerator("field \"PageIsOnByDefault\" of the StoredPageMatchReadCommand class");
